...
function HomeContent() {
  ...
  const [selectedModel, setSelectedModel] = useState<'gemini' | 'agent' | 'mcp-style' | 'n8n' | 'rag'>('gemini');
  const [showModelSelector, setShowModelSelector] = useState(false);
  const modelSelectorRef = useRef<HTMLDivElement>(null);
  
  // Active feature state
  const [activeFeature, setActiveFeature] = useState<'chat' | 'sql' | 'excel' | 'game' | 'image' | 'rag'>('chat');
  ...
  useEffect(() => {
    ...
    if (showModelSelector || isUserMenuOpen) {
  ...
  }, [showModelSelector, isUserMenuOpen]);
  ...
  return (
    <div className="flex h-screen bg-white dark:bg-[#212121] overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
        {/* Header */}
        <header className="bg-white dark:bg-[#212121] px-6 py-4 shadow-sm flex justify-between items-center flex-shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900 dark:text-white">
                ...
                {activeFeature === 'rag' && '📚 Document Q&A'}
              </h1>
              <p className="text-sm text-gray-600 dark:text-gray-300">
                ...
                {activeFeature === 'rag' && 'Upload Documents & Ask Questions with AI'}
              </p>
            </div>
            {activeFeature !== 'chat' && (
              <button
            ...
            {isUserMenuOpen && (
              <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-gray-900 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 py-2 z-50">
                ...
                <button
                  ...
                  className="w-full text-left px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-gray-100 dark:hover:bg-gray-800 flex items-center gap-3 transition-colors"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {messages.length === 0 ? (
              <div className="text-center text-gray-500 dark:text-gray-400 mt-20">
                ...
                <div className="mt-8 text-sm text-gray-600 dark:text-gray-500">
                  <div className="mb-2">✨ Supports rich content:</div>
